...
async def create_donation(
    ...
    donation = await donation_crud.create(donation, session, user=user)
    open_projects = await charity_project_crud.get_open_projects(session)
    invest(open_projects, [donation])
    await donation_crud.commit_and_refresh(session, [donation, *open_projects])
    return donation
